...
void Harris_filter::Update(Sobel_filter &sobel_filter, Event &event)
{
	...
	for (int u = 0; u < 3; u++)
	{
		...
		if (x >= 0 && x < sobel_filter.m_mat.cols)
		{
			for (int v = 0; v < 3; v++)
			{
				...
				{
					sum_ix2 += sobel_filter.m_mat.at<cv::Vec3d>(event.m_y, event.m_x)[0] * sobel_filter.m_mat.at<cv::Vec3d>(event.m_y, event.m_x)[0];
					sum_iy2 += sobel_filter.m_mat.at<cv::Vec3d>(event.m_y, event.m_x)[1] * sobel_filter.m_mat.at<cv::Vec3d>(event.m_y, event.m_x)[1];
					sum_ixy += sobel_filter.m_mat.at<cv::Vec3d>(event.m_y, event.m_x)[0] * sobel_filter.m_mat.at<cv::Vec3d>(event.m_y, event.m_x)[1];
				}
			}
		}
	}
	...
	double harris_response = (determinant - k * pow(trace, 2)) / 10;
