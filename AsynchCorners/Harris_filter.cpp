...
/*
void Harris_filter::Update2(Sobel_filter &sobel_filter, double timestamp)
{
	for (int i=0; i < sobel_filter.m_mat.cols; i++)
		for (int j = 0; j < sobel_filter.m_mat.rows; j++)
		{
			double sum_ix2 = 0;
			double sum_iy2 = 0;
			double sum_ixy = 0;
			for (int u = 0; u < 3; u++)
			{
				...
				if (x >= 0 && x < sobel_filter.m_mat.cols)
				{
					for (int v = 0; v < 3; v++)
					{
						...
						if (y >= 0 && y < sobel_filter.m_mat.rows)
						{
							sum_ix2 += sobel_filter.m_mat.at<cv::Vec3d>(j, i)[0] * sobel_filter.m_mat.at<cv::Vec3d>(j, i)[0];
							sum_iy2 += sobel_filter.m_mat.at<cv::Vec3d>(j, i)[1] * sobel_filter.m_mat.at<cv::Vec3d>(j, i)[1];
							sum_ixy += sobel_filter.m_mat.at<cv::Vec3d>(j, i)[0] * sobel_filter.m_mat.at<cv::Vec3d>(j, i)[1];
						}
					}
				}
			}
			...
			m_mat.at<double>(j, i) = harris_response / 10;
		}
}
*/